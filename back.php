...
{
	$No=$_POST['No'];
	$sql="UPDATE examfees SET fees = NULL  WHERE No='".$No."'LIMIT 1";
	$sql1="SELECT Tid from paid WHERE No='".$No."'";
	$rec=mysql_query($sql);
	$rec1=mysql_query($sql1);
	$row=mysql_fetch_assoc($rec);
	if (!$rec1) { // add this check.
  ...
  <main class="hoc container clear"> 
    ...
    <input type="text" placeholder="Enter Username" name="No" required>
		<!--<button input type="submit" >GO</button>-->
		 <input type="submit" name="insert" value="CHECK" onclick="select()" />
...
<?php
	...
	{
	{?>
	<p>SUCCESSFULLY FEES PAID WITH TRANSACTION ID: </p> <?=nl2br($row['Tid']); ?>
...
<input type=button onClick="location.href='fw.php'" value='BACK TO HOME'>
